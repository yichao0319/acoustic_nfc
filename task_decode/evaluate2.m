...
for si = [5]
  ...
  gt_symbol = load(['sent_pkt' int2str(seed) '.symbol']);
  gt_demod  = load(['sent_pkt' int2str(seed) '.demod']);
  ...
  for fi = [1:length(expnames)]
    expname = expnames{fi};
    filename = [char(expname) int2str(seed) '.wav'];

    if exist(['./output/' char(expname) int2str(seed) '.h.txt'], 'file') == 2
      continue;
    end
    ...
    % result = input('');
    dlmwrite(['./output/' char(expname) int2str(seed) '.h.txt'], Hfft, 'delimiter', '\t');
    dlmwrite(['./output/' char(expname) int2str(seed) '.symbol.txt'], data_equalized, 'delimiter', '\t');
    dlmwrite(['./output/' char(expname) int2str(seed) '.demod.txt'], data_demod, 'delimiter', '\t');
...
dlmwrite(['./output/ber.txt'], BERS, 'delimiter', '\t');
